...
import { applyPush, currentRev, pull, stats } from '../db/sync.ts';
...
export function registerSyncRoutes(app: FastifyInstance): void {
  app.post<{ Body: PushRequest }>(
    ...
    async (request, reply) => {
      ...
      if (size === 0) {
        return reply.send({ results: [], cursor: currentRev(context.accountId), serverTime: Date.now() });
      }
      if (size > 2000) {
      ...
      return reply.send({ results, cursor, serverTime: Date.now() });
    },
  );
  ...
  app.get<{ Querystring: { since?: string; limit?: string } }>(
    ...
    async (request, reply) => {
      ...
      return reply.send({ ...result, serverTime: Date.now() });
    },
  );

  app.get('/sync/status', { preHandler: requireDevice }, async (request) => {
    const context = device(request);
    return {
      cursor: currentRev(context.accountId),
      counts: stats(context.accountId),
      serverTime: Date.now(),
    };
  });
}
