 ...
 *
 * The server runs the same prompt and the same normaliser, so results are
 * identical to calling the provider directly — only the trust boundary moves.
 */

import { normalizeExtraction } from '@kvitto/shared';
...
export const serverProvider: Provider = {
  ...
  async extract(request: ExtractionRequest): Promise<ExtractionResponse> {
    ...
    return {
      extraction: normalizeExtraction(payload.raw),
      raw: payload.raw,
