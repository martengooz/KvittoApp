/**
 * The cache-first company lookup, and the daily budget policy that guards its
 * weaker, name-based path.
 *
 ...
 *
 * This sits above `db/companies.ts` on purpose: it reaches into `core/settings`
 * for the API key and quota, and into `api/apiverket` to make the call, which
 * a `db/` module has no business doing. The plain store — reading and writing
 * `Company` rows — stays there; this is the policy in front of it.
 */
