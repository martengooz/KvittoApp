 ...
 *
 * This lives in `shared` rather than in either app because both need it and
 * neither owns it. It touches no DOM and no React Native API - `fetch`,
 * `AbortSignal` and `Response` are all either app has - so the only thing that
 * had to change on the way here was recognising an abort by `error.name`
 * instead of `instanceof DOMException`, which Hermes does not define.
 */

import { checkOrgNumber } from './orgnumber.js';
...
async function request<T>(path: string, options: LookupOptions): Promise<T> {
  ...
  try {
    ...
    // Checked by name rather than by `instanceof DOMException`: Hermes has no
    // `DOMException`, so the instance check is always false on the phone and
    // an abort would be reported as an unreachable registry.
